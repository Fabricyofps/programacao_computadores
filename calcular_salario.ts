...
rl.question(
  ...
  (inputHoras) => {
    ...
    rl.question("Digite a taxa por hora: ", (inputTaxa) => {
      ...
      if (
        isNaN(horasTrabalhadas) ||
        isNaN(taxaPorHora) ||
        horasTrabalhadas < 0 ||
        taxaPorHora < 0
      ) {
        console.log(
          "Por favor, insira valores válidos e não negativos para horas trabalhadas e taxa por hora."
        );
      } else {
        const salarioSemanal = calcularSalarioSemanal(
          horasTrabalhadas,
          taxaPorHora
        );
